defmodule AppPhoenix.UserControllerTest do
  ...
  @invalid_attrs %{}
  @valid_create_nonadmin %{email: "nonadmin@test.com", username: "nonadmin", password: "test", password_confirmation: "test"}
  @valid_create_admin %{email: "admin@test.com", username: "admin", password: "test", password_confirmation: "test"}

  setup do
    {:ok, user_role}     = TestHelper.create_role(%{name: "user", admin: false})
    {:ok, nonadmin_user} = TestHelper.create_user(user_role, @valid_create_nonadmin)
    {:ok, admin_role}    = TestHelper.create_role(%{name: "admin", admin: true})
    {:ok, admin_user}    = TestHelper.create_user(admin_role, @valid_create_admin)
    conn = conn()
